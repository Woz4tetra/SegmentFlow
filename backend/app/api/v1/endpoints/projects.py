...
@router.post("/projects/{project_id}/populate_images")
async def populate_images(
    project_id: UUID,
    db: AsyncSession = Depends(get_db),
) -> dict:
    """Populate Image records for existing converted frames.

    This is a utility endpoint for projects that were converted before
    the Image record creation was implemented.
    ...
    Returns:
        dict: Number of images created

    Raises:
        HTTPException: If project not found or no frames exist
    """
    try:
        # Verify project exists
        result = await db.execute(select(Project).where(Project.id == project_id))
        ...
        if not inference_dir.exists() or not output_dir.exists():
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Frame directories not found. Please convert video first.",
            )
        ...
        if not inference_files:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="No frames found in inference directory",
            )

        # Delete existing Image records for this project
        await db.execute(select(Image).where(Image.project_id == project_id))
        existing_images = (
            (await db.execute(select(Image).where(Image.project_id == project_id))).scalars().all()
        )
        ...
        await db.commit()
        logger.info(f"Created {created_count} Image records for project {project_id}")

        return {
            "project_id": str(project_id),
            "images_created": created_count,
            "message": f"Successfully created {created_count} image records",
        }
        ...
        await db.rollback()
        logger.error(f"Failed to populate images for project {project_id}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to populate images: {e!s}",
        ) from e
